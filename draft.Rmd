...
The key outcomes of interest for this reanalysis, the change in balance after six months and the change in balance after twelve months, are both highly right skewed. 
...
Here is the same information as a kernel density estimate (KDE). 
...
The right skew is present in the treatment and control groups, but it most pronounced in the commit-savings treatment group after twelve months. 
...
skewc = treated %>%
  ...
  geom_point(alpha=0.2) +
  scale_x_continuous(labels = scales::comma) +
  labs(title = "Variance by treatment having removed significant outliers",
       x = "Change in balance (Philippine Peso)",
       y = NULL) +  
  theme_minimal()
...
We are clearly comfortable with the first two assumptions. We are comfortable with the third assumption, because outliers aside, the we can see in the data exploration section that the three treatments are all tightly clustered around 0.
...
comparison_matrix = as.table(m <= adjusted_alpha)
knitr::kable(comparison_matrix, caption="Significant difference found in pairwise comparison with Wilcoxon Test.")
...
A p-value of `r kw_result$p.value` for the Tukey HSD permutation test leads us to reject the null hypothesis in favor of the alternative. We conclude that there is a difference in savings between at least two of the groups. 

The matrix shows that the SEED accounts had better savings than the control group, but not the marketing groups. No other differences were significant. This allows us to conclude that the SEED accounts had better savings than the control, but are not significantly more effective than the marketing groups. This is a contradiction to the original paper.
...
The dependent variable, change in savings account balance, is highly right-skewed after six months and twelve months as demonstrated in [todo(aaron): add plot number from above]. Accordingly, the dependent variable is not approximately normally distributed and the error term of their estimated models is not approximately normally distributed.

[todo(aaron): add plot number from above] The SEED treatment, the marketing control, and the full control all have outliers. Though the SEED treatment's outliers are distinct for the change in twelve month balance. 
...
The models estimated with OLS have residuals that are highly skewed. The following QQ plots demonstrate that many of the residuals are approximately normally distributed, as they fall on the diagonal line. However, the residuals dramatically depart the line at higher values. 
...
resid1 = augment(lm3) %>%
  ...
  geom_qq_line() +
  theme_minimal() +
  labs(title = "The Estimated Model has Significant Outliers",
