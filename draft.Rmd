...
Since we have reason to believe that marketing about savings will be more effective than no marketing, and that marketing plus a SEED account will be more effective than just marketing, we will use an ordered k-sample test. Here we use the Jonckheere-Terpstra test as recommended by Higgins. [@Higgins]

We will conduct this test at a significance level of $\alpha=0.05$ with the following hypothesis.

$H_0: \theta_x = \theta_y = \theta_z$

$H_a: \theta_x \leq \theta_y \leq \theta_z$ with at least one strict inequality
...
x = list(control_balchange, marketing_balchange, seed_balchange)
pJCK(x, method="Monte Carlo")
```
...
a_1 = calculate_critical(sample_q, 0.1)
a_05 = calculate_critical(sample_q, 0.05)
crit_message = sprintf("critical values of significance levels 0.1 and 0.05 are %s and %s respectively", a_1, a_05)
print(crit_message)
...
print("pairwise statistics greater than or equal to the 0.05 critical value")
print(m >= a_05)
```
...
A p-value of 0.0001 for the Jonckheere-Terpstra test means that we reject the null hypothesis in favor of the alternate. We conclude that there is a difference in savings between the three groups. 

A p-value of 0 for the Tukey HSD permutation test shows that the SEED account is awesome. The matrix shows that the SEED accounts had better savings than the control group and the marketing groups. No other differences were significant.
