...
We conclude using paired comparisons that customers who had SEED accounts saw an increase in savings over the duration of the trial. We conclude using Tukey Permutation Honest Significant Difference tests that SEED accounts had more savings than the other two groups. Finally, using M-Estimation and rank-based and robust regression we find that SEED accounts had more savings than the other two groups; however, we find much smaller effect sizes than [@Ashraf2006].
...
The right skew is present in the treatment and control groups, but it most pronounced in the commit-savings treatment group after twelve months as demonstrated in \textit{figure 3}. 
...
We are clearly comfortable with the first two assumptions. We are comfortable with the third assumption, because outliers aside, the we can see in Figure 4 that three treatments have similar spreads after the significant outliers are removed.
...
If we were doing a pairwise comparison test to determine the existance of a difference, we would use Bonferroni's adjustment for the paired tests to control for type one error. Giving an adjusted alpha of $\alpha' = \frac{2\alpha}{k(k-1)} = `r 2 * 0.05 / (3 * (3 - 1))`$[@Higgins].  However, we will only be using the pairwise comparison after the k-sample Kruskal-Wallis test, so we do not need to be concerned with it here.
...
A p-value of `r fp_result$p.val` for the Fligner-Policello Test means that we reject the null hypothesis in favor of the alternate. We conclude that the median savings for SEED accounts is higher an for non-SEED accounts.
...
Tables 4 and 5 show a significant difference in three one-sided comparisons tests. We can reasonably conclude that the SEED accounts perform better than the control groups and the marketing treatment. However, it is worth noting that if we used the Bonferroni adjustment for alpha, we would only mark a significant difference between the seed and control groups.
...
In most cases our findings are aligned with the original paper. In section 3 we conclude that the SEED accounts are effective savings mechanisms, agreeing with the original authors. In section 4 we conclude that there is a difference between the three different treatments, but make a note that depending on the significance level employed we have reason to doubt that there is a difference between the marketing treatment and the SEED accounts. The tests concluded a difference, but further study is warranted. In section 5 we used different regression techniques from the original paper, as the authors had done a good bit of massaging to make OLS work. While the non parametric methods did conclude statistical siginificance of the SEED treatments over the marketing treatments, the coefficients determined by the methods were significantly lower. Leading us to conclude that while there is a statistically significant effect of the SEED accounts on savings compared to the marketing treatment, there is not necessarily a practically significant effect.

Overall, we conclude that the SEED accounts are effective saving mechanisms, with some marginal benefits over marketing treatments. However, like the authors of the original paper, we also recommend further study into the true effectiveness of the accounts, and how large the difference is between them and other potential marketing strategies.
